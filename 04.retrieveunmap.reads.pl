...
my @sams = sort(grep(/^[A-Z]\w+/, readdir(REF)));
my @resams = @sams;
close REF;
...
system("mkdir -p 04.retrieve.reads/03.bowtie.nucl/99999");
#foreach my $sam (@resams){
	#system("mv $reffolder/$sam/$sam.tmp.$cur.* $srcfolder/99999/");
